...
def get_pascal_voc_metrics(gold_standard: List[BoundingBox],
    ...
    for c in classes:
        ...
        for i in range(len(preds)):
            ...
            if gt_box.get(preds[i].image_name) == []:
                gt_box[preds[i].image_name] = gt
            ...
            if mas_idx == -1:
                ...
                if gt_box[preds[i].image_name][mas_idx] is None:
                    pass
                else:
                    preds[i].index = gt_box[preds[i].image_name][mas_idx].index

            # Assign detection as true positive/don't care/false positive
            if max_iou >= iou_threshold:
                if counter[preds[i].image_name][mas_idx] == 0:
                    tps[i] = 1  # count as true positive
                    if tp_box.get(preds[i].image_name) is None:
                        tp_box[preds[i].image_name] = []
                    tp_box[preds[i].image_name].append(preds[i])
                    counter[preds[i].image_name][mas_idx] = 1  # flag as already 'seen'
                    gt_box[preds[i].image_name][mas_idx] = None
                else:
                    # - A detected "cat" is overlaped with a GT "cat" with IOU >= IOUThreshold.
                    preds[i].index = -1
                    fps[i] = 1  # count as false positive
                    fp_box[preds[i].image_name].append(preds[i])
            else:
                fps[i] = 1  # count as false positive
                if fp_box.get(preds[i].image_name) is None:
                    fp_box[preds[i].image_name] = []
        ...
        r.recall = recalls
        r.ap = ap
        r.interpolated_recall = np.array(mrec) if mrec is not None else None
        r.interpolated_precision = np.array(mpre) if mpre is not None else None
        r.tp = np.sum(tps)
        r.fp = np.sum(fps)
        r.array_fn = gt_box
        r.array_fp = fp_box
        r.array_tp = tp_box
        r.tps = cumulative_tps
        r.fps = cumulative_fps
        r.preds = preds
